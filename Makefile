...
$(JSON_PATH)/phewas_catalog.json.gz:
	mkdir -p $(JSON_PATH)
	curl --silent https://storage.googleapis.com/ot-releases/18.12/evidences/phewas_catalog-28-11-18.json.gz | gunzip -c -- | shuf -n $(NUMBER_TO_KEEP) | gzip > $(JSON_PATH)/phewas_catalog.json.gz

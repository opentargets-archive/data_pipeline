...
#CTTV specific parser, modified from the BioPython one.
...
class Parser(object):
    ...
    def parse(self):
        ...
        def _parse_complex_dbxref(element):
            key= element.attrib['id']
            properties = {}
            for ref_element in element:
                if ref_element.tag == NS + 'property':
                    try:
                        properties[ref_element.attrib['type']]=ref_element.attrib['value']
                    except:
                        pass
            if properties:
                ...
                self.ParsedSeqRecord.annotations['dbxref_extended'][element.attrib['type']][key]=properties
    ...
    def _parse_complex_dbxref(self, element):
        pass
